...
# - Graduation Rate: The percentage of students who graduate within
#   4 years.
#
...
# %%
job_placement_url = (
    'https://raw.githubusercontent.com/DG1606/CMS-R-2020/master/'
    'Placement_Data_Full_Class.csv'
)
job_placement = pd.read_csv(job_placement_url)
job_placement.head()
...
college_completion['control'] = (
    college_completion['control'].astype('category')
)
college_completion['level'] = (
    college_completion['level'].astype('category')
)
college_completion['basic'] = (
    college_completion['basic'].astype('category')
)
college_completion['state'] = (
    college_completion['state'].astype('category')
)
...
drop_cols_cc = [
    ...
    'vsa_year', 'vsa_grad_after4_first',
    ...
    'similar', 'counted_pct', 'hbcu', 'flagship',
    'med_sat_value', 'med_sat_percentile'
]
college_completion = college_completion.drop(columns=drop_cols_cc)
# These columns do not contribute to the analysis and may introduce noise.
# Some have lots of missing values, some are identifiers, and some are not
# relevant.
college_completion.info()

# %%
# One-hot encoding factor variables:
college_completion = pd.get_dummies(
    college_completion,
    columns=['control', 'basic', 'state', 'level'],
    drop_first=True
)
# This will create new binary columns for each category in the categorical
# variables.
...
# Not going to normalize our target variables (graduation rates) since
# they are already normalized as percentages.
scaler = StandardScaler()
college_completion[continuous_vars] = scaler.fit_transform(
    college_completion[continuous_vars]
)
# This will standardize the continuous variables to have a mean of 0 and a
...
college_completion['high_grad_rate'] = (
    (college_completion['grad_150_value'] > 60).astype(int)
)

# %%
# Calculate the prevalence of the target variable:
prevalence = (
    college_completion['high_grad_rate'].sum() /
    len(college_completion)
)
...
X = college_completion.drop(
    columns=['grad_150_value', 'high_grad_rate']
)
# Assuming the model will be classification, we only need the binary target
# variable.
...
# They look to all be binary categorical variables, so we can convert them
# to category type:
job_placement['gender'] = job_placement['gender'].astype('category')
job_placement['specialisation'] = (
    job_placement['specialisation'].astype('category')
)
...
# These are all categorical variables, let's convert them:
...
# %%
# Collapse factor levels as needed:
# All of our categorical variables have 2 or 3 categories, so we don't need
# to collapse any.

# %%
# One-hot encoding factor variables:
categorical_vars = [
    'gender', 'specialisation', 'status', 'workex', 'degree_t',
    'hsc_b', 'hsc_s', 'ssc_b'
]
job_placement = pd.get_dummies(
    job_placement, columns=categorical_vars, drop_first=True
)
...
job_placement[continuous_vars_jp] = scaler.fit_transform(
    job_placement[continuous_vars_jp]
)
...
# Only salary has missing values, so we can drop it since it's not a
# predictor variable:
...
job_placement['placed'] = (
    (job_placement['status_Placed'] == 1).astype(int)
)
...
# Same as before, we will first separate train_tune from test, then split
# train_tune into train and tune sets.
